...
> 将100+问题集按用户意图划分为 5 个类别，统计截止目前，MindSpore 社区各类问题的引用覆盖率。

### 引用趋势变化（Round 1 → Round 2，任一平台引用即视为引用）

4.20 GEO 改进措施上线后，整体引用覆盖率由 **77% 提升至 84%**。分平台看，DeepSeek 52%→58%，Qwen 48%→55%，两平台均有改善且幅度相近。

### 类别总览

| 意图类别 | 双平台引用 | 任一平台引用 | DeepSeek | Qwen |
|---------|:--------:|:--:|:--------:|:----:|
| 邮件列表与通信 | 100% | 100% | 100% | 100% |
| 社区参与与活动 | 50% | 100% | 75% | 75% |
| SIG 与治理 | 43% | 100% | 43% | 100% |
| 技术开发与调试 | 0% | 69% | 54% | 15% |
| 安装与配置 | 25% | 75% | 50% | 50% |
| **合计** | **29%** | **84%** | **58%** | **55%** |
...
### 现状
**邮件列表与社区内容被两平台均衡引用**：`mailweb.mindspore.cn` 和 `mindspore.cn/activities`、`/contribution` 均被双平台稳定引用，是当前内容 GEO 效果最好的区域。**安装和配技术开发与调试类内容**仍是短板。
...
5. **双平台引用率整体偏低，两平台引用来源高度分化**：尽管任一平台引用率达 84%，但双平台同时引用的题目仅 **29%**，说明 DeepSeek 与 Qwen 在官方内容的引用选择上存在显著分歧。其中针对技术类内容，DeepSeek 和 Qwen 引用**无任何重叠**，DeepSeek 倾向引用官方文档和 FAQ，Qwen 几乎不引用技术文档，转而使用社区问答或第三方来源。这一分化现象意味着：即使某平台已引用官方内容，另一平台的用户仍可能获取不到官方信息，内容 GEO 的改进需要同时兼顾不同平台的索引偏好，而非仅关注整体引用率。
