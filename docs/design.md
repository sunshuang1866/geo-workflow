...
问题集分类的核心维度是"这个问题需要什么类型的答案"，这决定了官网应该在哪个路径下有什么内容。
...
**T（教程）**
- 快速入门（Quick Start）：最小可运行示例，5–10 分钟完成
- 端到端场景教程：从零完成一个完整任务（如"用 MindSpore 训练第一个模型"）
- 安装 + 首次使用完整流程（面向新用户的第一步引导）
- 动手实验（Codelab / Lab）

**H（操作指引）**
- 特定功能配置指南（如"配置 TLS 证书"、"开启审计日志"）
- 集成指南（如"与 Kubernetes 集成"、"对接 Prometheus 监控"）
- 迁移指南（如"从 CentOS 迁移到 openEuler"）
- 升级指南（版本间升级步骤）
- 特定环境部署（如"在 ARM 架构上部署"、"离线环境安装"）
- 进阶操作（高级配置、性能调优、二次开发）

**R（参考文档）**
- API 参考（REST API、Python/Java/Go SDK 接口说明）
- CLI 命令参考（所有子命令、参数、示例）
- 配置文件全量参数说明（字段名、类型、默认值、取值范围）
- 环境变量列表
- 错误码对照表
- 术语表（Glossary）

**E（概念解释）**
- 架构总览（系统由哪些组件构成、各组件如何协作）
- 核心概念介绍（"什么是 X"、"X 的工作原理是什么"）
- 设计原则与决策背景（为什么这样设计）
- 产品对比（"X 与 Y 的区别与适用场景"）
- 功能深度解析（某特性的设计理念和适用边界）
- 安全模型、数据流说明等背景性技术文章

**F（排障/FAQ）**
- FAQ 页面（高频问题 + 简短答案的列表式页面）
- 按症状分类的故障排查指南（如"服务启动失败的排查步骤"）
- 错误信息解释（报错现象 → 原因 → 解决方案，三段式）
- 已知问题列表（Known Issues）及临时规避方案
- 常见陷阱与注意事项（Gotchas / Pitfalls）
- 社区高频问题结构化整理（来自 issue / 论坛帖子的提炼版本）
...
| 状态 | 条件 | 含义 | 后续处理 |
|---|---|---|---|
| `satisfied` | `cited_count ≥ 1`（至少 1 个平台引用了官方内容）| GEO 表现正常 | Issue 标记 `[已解决]` |
| `not_cited` | 有 `official_urls`，但所有平台均未引用 | 官网有内容，AI 不引用 — 内容结构或爬取可见性问题 | 按 doc_form 诊断后生成改进 Issue |
| `no_official_content` | `official_urls` 为空 | 官网缺少对应内容 — 需新建页面 | 按 doc_form 确定应新建的内容类型和 URL，生成补充 Issue |

#### 6.2.1 not_cited 诊断路径（按 doc_form）

`not_cited` 说明内容存在但 AI 未能引用。由于 AI 平台均开启了网页搜索，失效原因集中在**内容结构**（缺 schema.org 标注）和**爬取可见性**（JS 渲染 / sitemap / robots.txt）两个方向，具体按 doc_form 区分：

| doc_form | 常见失效原因 | 首先检查 | 然后检查 |
|---|---|---|---|
| T / H | 步骤混在长文中；H1 无动词关键词；缺 schema 标注 | H1/H2 标题是否含任务动词 + 核心关键词 | 是否用 `HowTo` schema 标注每个步骤 |
| R | 页面 JS 动态渲染，爬虫无法抓取 | 页面是否服务端渲染（SSR） | 是否已加入 sitemap.xml |
| E | 内容散落在长文或博客中，无独立页面；H1 非问题句式 | 是否有稳定 URL 的独立概念页 | H1 是否采用问题句式（"什么是 X"/"X 的原理是什么"）|
| F | 缺独立 FAQ 页；无 `FAQPage` schema 标注 | 是否有 `FAQPage` schema.org 标注 | 问题文本是否同时覆盖中英文表达 |

#### 6.2.2 no_official_content 内容补充优先级（按 doc_form）

`no_official_content` 说明官网尚无对应内容，需新建页面。按 doc_form 确定应创建的内容类型和目标路径：

| doc_form | 应创建的内容 | 目标 URL 路径 |
|---|---|---|
| F | FAQ 页或故障排查指南（优先级最高，改进性价比最高）| `/faq/` 或 `/troubleshooting/` |
| H | 对应任务的操作指引（进阶操作最常缺失）| `/docs/how-to/{topic}/` |
| E | 独立概念页（避免散落在博客中）| `/docs/concepts/{topic}/` |
| T | 快速入门或端到端教程（精简为最小可运行示例）| `/tutorials/` 或 `/getting-started/` |
| R | API / CLI / 配置参数参考文档 | `/reference/{api-or-config}/` |
