...
- `questions.json` — source of `official_urls` and `notes` per question (from `output/{community}/`)
...
| `questions_file` | no | `output/{GEO_COMMUNITY}/questions.json` | Path to questions file; `{GEO_COMMUNITY}` resolved from `.env` |
...
```json
{
  "metadata": {
    ...
    "source_files": {
      ...
      "questions": "output/{community}/questions.json",
      "issue_map": "issue-map.json",
      "prev_report": "output/MindSpore/2026-03-23/assessment-report.json"
    }
  },
  "changes": {
