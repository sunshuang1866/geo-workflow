...
**AI 平台：** ChatGPT · DeepSeek · 豆包 · Qwen · Gemini · Kimi（按 `.env` 中有效 API Key 动态启用）
...
flowchart TD
    ...
    subgraph PREFILL["内容审计层"]
        PF["prefill-urls<br>三层 URL 审计<br>+ doc_form 路径校验<br>+ 人工校核 note"]
    end

    subgraph PLATFORMS["AI 平台采样（6个）"]
        P1["ChatGPT"]
        P2["DeepSeek"]
        P3["豆包"]
        P4["Qwen"]
    ...
    subgraph OUTPUT["输出层"]
        O1["GitHub / GitCode Issues<br>（每题一个 Issue）"]
        O2["assessment-report.md<br>（人工可读评估报告）"]
        O3["run-meta.json<br>（运行元数据）"]
    end

    subgraph COMPANION["配套 Workflow（内容改进）"]
        C1["分析 GEO Issues<br>→ 修改官网内容<br>→ 提交 PR"]
    end

    A1 & A2 --> PIPELINE
    S1 & S2 & S3 & S4 & S5 -->|"/get-question<br>生成 questions.json + doc_form"| B0
    B0 -->|"questions.json"| PF
    PF -->|"official_urls + note 填充<br>人工校核后"| B1
    B1 -->|"并发采样"| P1 & P2 & P3 & P4 & P5 & P6
    P1 & P2 & P3 & P4 & P5 & P6 -->|"responses.json"| B2
    B2 -->|"scoring-results.json"| B3
    B3 --> O1
    B4 --> O2
    B5 --> O3
    O1 -->|"触发"| C1
    C1 -->|"PR 合入后重新触发 GEO 检测"| A1
```
...
| **问题来源层** | 从社区真实数据中提取代表性搜索问题，推断 doc_form | `get-question` Skill，4 条自动化路径 + 手动标注 |
| **内容审计层** | 审计官网 URL 质量，标注 doc_form 路径合理性 | `prefill-urls` Skill，三层判断 + 人工校核 |
| **编排层** | 流水线控制，参数化执行 | `AGENT.md` |
| **AI 平台层** | 向 6 个 AI 平台并发发送问题并收集回答 | `platform-chat` Skill，Playwright Web UI 采样 |
| **评分与行动层** | URL 匹配评分 → doc_form 诊断 → 每题一 Issue | `scoring-engine` + `issue-creator` + `assessment-report` |
| **配套改进层** | 读取 GEO Issues → 修改官网 → PR 合入触发下一轮检测 | 内容改进 workflow（独立项目） |
...
│
├── output/{community}/           # 评估数据（按社区 + 日期隔离）
...
│   ├── get-question/                  # 问题集生成（含 doc_form 推断）
│   │   ├── SKILL.md                   # 8 步过程定义
│   │   ├── scripts/                   # Python 脚本（5 个）
│   │   ├── references/                # API 规范（3 个）
│   │   └── assets/                    # 模板和提示词
│   ├── prefill-urls/                  # 内容审计与链接预填
│   │   ├── SKILL.md                   # 三层判断过程定义
│   │   ├── scripts/                   # Python 脚本（URL 验证、路径校验）
│   │   ├── references/                # doc_form 路径规范
│   │   └── assets/                    # 输出模板
│   ├── platform-chat/                 # AI 平台采样（6 平台 Web UI）
│   │   ├── SKILL.md                   # 5 步过程定义
...
```
get-question ──► questions.json（含 doc_form）
                      │
                      ├──► prefill-urls ──► questions.json（含 official_urls + note）
                      │                              │
                      │                              ├──► platform-chat ──► responses.json
                      │                              │
                      │                              ├──► scoring-engine ──► scoring-results.json
                      │                              │                              │
                      │                              │                              ├──► issue-creator ──► GitHub/GitCode Issues（每题一个）
                      │                              │                              │        │
                      │                              │                              │        └──► issue-map.json
                      │                              │                              │
...
| 模式 | 应用场景 |
|------|---------|
| **管道-过滤器** | get-question → prefill-urls → platform-chat → scoring-engine → issue-creator 单向流水线 |
...
| **doc_form 分类** | 问题按内容形式（T/H/R/E/F）分类，驱动 URL 路径校验、不引用诊断路径、Issue 改进建议 |
| **双 Workflow 对抗** | GEO 检测 workflow 产出 Issues，内容改进 workflow 消费 Issues 产出 PR，PR 合入触发下一轮检测 |
...
flowchart LR
    subgraph LOCAL["本地环境"]
        ...
        B["output/{community}/{date}/<br>JSON 数据文件"]
    ...
    subgraph COMPANION["配套 Workflow（独立项目）"]
        G1["内容改进 Workflow<br>读取 GEO Issues<br>→ 修改官网内容<br>→ 提交 PR"]
    end

    A -.->|"HTTPS · 采样提问"| D1 & D2 & D3 & D4 & D5
    A -.->|"HTTPS · 问题采集"| E1 & E2 & E3 & E4
    A -.->|"HTTPS · Issue CRUD"| F1 & F2
    F1 & F2 -.->|"GEO Issues 触发"| G1
    G1 -.->|"PR 合入后触发新一轮检测"| A
```
...
| **数据存储** | 本地 JSON 文件 | `output/{community}/{date}/`，按日期隔离 |
...
用户: "对 MindSpore 生成问题集并完成首次 GEO 评估"
    │
    ├─► Step 0: init — 创建 output/MindSpore/2026-04-14/ 目录
    │
...
#### 场景 4：双 Workflow 对抗闭环

```
本 Workflow（GEO 检测）：
    get-question → prefill-urls → platform-chat → scoring-engine → issue-creator
    └─► 输出：GitHub Issues（含【GEO】标记，每题一个）

配套 Workflow（内容改进，独立项目）：
    读取【GEO】Issues → 分析改进建议 → 修改官网内容 → 提交 PR → PR 合入

PR 合入触发新一轮 GEO 检测：
    └─► 对比前后 scoring-results.json，量化：
          - 问题级：citation_rate 变化（before/after）
          - 平台级：哪些平台开始引用了官方内容
...
- LLM 在改写标题的同时推断 `doc_form`（T/H/R/E/F），写入每条问题记录
- 输出：`questions.json`（机器可读，含 doc_form）+ `questions.md`（人工可读）

---

### 3.3 内容审计与链接预填（prefill-urls）

```
.claude/skills/prefill-urls/
├── SKILL.md                        # 三层判断过程定义
├── scripts/
│   ├── discover-docs.py            # 在官网搜索与问题相关的页面
│   ├── validate-urls.py            # HTTP HEAD 验证 URL 可访问性
│   ├── search-discourse.py         # Discourse 搜索补充官方链接
│   └── expand-i18n-urls.py         # 中英文 URL 路径扩展
├── references/
│   └── doc-form-path-patterns.md   # 各 doc_form 的期望 URL 路径规范
└── assets/
    └── prefill-template.md         # 输出模板
```

**三层判断逻辑：**

```
对 questions.json 中每条问题：

第一层：官网是否有相关内容？
  有 → 进入第二层
  无 → official_urls: [], note: "没有链接"

第二层：URL 路径是否符合 doc_form 期望路径模式？
  符合 → 填入 official_urls, note: ""
  不符合 → 填入当前可用 URL, note: "有链接但位置不合适：[当前] → 建议 [期望]"

第三层：HTTP HEAD 验证
  不可访问 → 不写入 official_urls, note: "链接无法访问：[URL]（404）"
```

**doc_form 与期望 URL 路径对应关系：**

| doc_form | 名称 | 期望路径模式 | schema.org 类型 |
|----------|------|------------|----------------|
| T | Tutorial | `/tutorials/`、`/getting-started/` | HowTo |
| H | How-to | `/docs/how-to/`、`/guides/` | HowTo |
| R | Reference | `/reference/`、`/api/` | TechArticle |
| E | Explanation | `/concepts/`、`/architecture/` | Article |
| F | Troubleshooting/FAQ | `/troubleshooting/`、`/faq/` | FAQPage |

**人工校核节点：**  
AI 填写完成后，`note` 非空的问题需人工校核，确认判断准确后 questions.json 方可作为 platform-chat 输入。

---

### 3.4 AI 平台采样（platform-chat）

```
.claude/skills/platform-chat/
├── SKILL.md                        # 5 步过程定义
├── scripts/
│   ├── run-platform-chat.py        # 批量入口（5 问题/批，平台间 1s 间隔）
│   ├── ask-chatgpt.py              # ChatGPT Web UI 采样（Session Token）
│   ├── ask-deepseek.py             # DeepSeek Web UI 采样（自动登录）
│   ├── ask-qwen.py                 # Qwen Web UI 采样（自动登录）
│   ├── ask-gemini.py               # Gemini Web UI 采样（匿名）
│   ├── check-env.py                # 前置检查（playwright + Chromium + session）
│   └── verify-session.py           # session 文件有效性验证
├── references/
...
→ 按 doc_form 匹配不引用诊断路径（见下表）
→ 写入 scoring-results.json
→ 同步 official_urls 回 questions.md（新增「官方链接」列）
```

**not_cited 按 doc_form 的诊断路径：**

| doc_form | 常见失效原因 | 首先检查 | 然后检查 |
|----------|------------|---------|---------|
| T / H | 步骤混在长文中；H1 无动词关键词；缺 schema 标注 | H1/H2 是否含任务动词 + 核心关键词 | 是否用 `HowTo` schema 标注每个步骤 |
| R | 页面 JS 动态渲染，爬虫无法抓取 | 是否服务端渲染（SSR） | 是否已加入 sitemap.xml |
| E | 内容散落在长文或博客中；H1 非问题句式 | 是否有稳定 URL 的独立概念页 | H1 是否采用问题句式 |
| F | 缺独立 FAQ 页；无 `FAQPage` schema 标注 | 是否有 `FAQPage` schema.org 标注 | 问题文本是否覆盖中英文表达 |

---

### 3.6 Issue 创建与追踪（issue-creator）
...
每个 question_id 独立对应一个 Issue，便于精确追踪改进状态、关联 PR 与效果、支持看板可视化。
...
scoring-results.json（本次评分）
    ↓
与 issue-map.json 比对（match key: question_id 精确匹配）
    ├── 无匹配 → 创建新 Issue → 写入 issue-map.json
    ├── 有匹配 + status 变化 → 追加评论（含 citation_rate 前后对比）
    └── 有匹配 + status = satisfied → 追加评论 + 标题加 [已解决] 前缀
→ 写 created-issues.json（本次活动日志）
```

---

### 3.7 评估报告（assessment-report）
...
AGENT.md Step 0 (init)
    ├── 读取 .env → GEO_COMMUNITY=MindSpore, GEO_COMMUNITY_DIR=output/MindSpore/
    ├── 比对 questions.json 与上次运行快照 → 无变更，继续
    └── 创建 output/MindSpore/2026-04-14/ 目录
    │
    ▼
AGENT.md Step 1 (sample) → /platform-chat
    ├── 加载 output/MindSpore/questions.json（42 个问题，已含 official_urls + note）
    ├── 检测 .env 中有效凭证: deepseek ✅, qwen ✅, gemini ✅, chatgpt ✅, kimi ✅, doubao ❌
    ├── 分 9 批（每批 5 题）× 5 平台 = 210 次 Web UI 采样
    └── 输出 output/MindSpore/2026-04-14/responses.json（210 条，3 条 error）
    │
    ▼
AGENT.md Step 2 (score) → /scoring-engine
    ...
    └── 输出 output/MindSpore/2026-04-14/scoring-results.json
    │
    ▼
AGENT.md Step 3 (issue) → /issue-creator
    ├── 读取 scoring-results.json + output/MindSpore/issue-map.json（已有 30 个问题 Issue）
    ├── 逐题比对 issue-map.json（match key: question_id 精确匹配）
    ├── 28 题匹配已有 Issue（追加评论），2 题无匹配（新建 Issue）
    ├── 调用 GitHub API: POST /issues × 2, POST /issues/*/comments × 28
    └── 更新 issue-map.json，写 output/MindSpore/2026-04-14/created-issues.json
    │
    ▼
AGENT.md Step 4 (report) → /assessment-report
    ...
    └── 输出 output/MindSpore/2026-04-14/assessment-report.json + .md
    │
    ▼
AGENT.md Step 5 (finalize)
    ├── 验证 issue activity（created-issues.json 统计与 run-meta 一致）
    ├── 写入 output/MindSpore/2026-04-14/run-meta.json
...
# 首次完整运行
/get-question
# → LLM 自动推断 doc_form，追加问题到 questions.json
/prefill-urls
# → AI 三层审计 official_urls + note，人工校核 note 非空项
/platform-chat
/scoring-engine
...
### 9.4 Issue 粒度：每问题一个 Issue，不做语义分组

**决策背景：** v1.0 设计中 LLM 对相关问题做语义分组，一组问题共享一个 Issue，以减少 Issue 数量。

**改为每题一 Issue 的原因：**
- 精确追踪：每个 question_id 与一个 Issue 一一对应，PR 合入后可精确量化该问题的 citation_rate 变化
- 避免语义分组漂移：LLM 分组结果不稳定，跨运行分组边界可能变化导致 issue-map.json 匹配失败
- 看板可视化：每张卡片对应一个可观测单元，状态（open/closed/labeled）直接反映 GEO 改进进度

**已知代价：** 问题数量多时 Issue 数量相应增加，需在 Issue 标题中嵌入 doc_form 和 status 标签以便筛选。

---

### 9.5 双 Workflow 对抗机制

**决策背景：** GEO 检测产出 Issues 后，改进行动由人工判断和执行，改进效果缺乏量化反馈。

**引入配套 workflow 的原因：**
- 改进效果可量化：PR 合入后自动触发下一轮 GEO 检测，对比 scoring-results.json 可得到精确的 citation_rate 变化
- 职责分离：GEO 检测 workflow 只负责"发现问题"，内容改进 workflow 只负责"修复问题"，互不干扰
- 形成闭环：两个 workflow 通过 Issues 和 PR 合入事件耦合，无需人工协调触发时机

---

### 9.6 禁止将 AI 生成的问题作为问题来源
...
*文档版本历史：v1.0 (2026-04-12) 初稿；v2.0 (2026-05-26) 合入 prefill-urls 新步骤、doc_form 分类、每题一 Issue 设计、Kimi 平台扩展、双 Workflow 对抗机制。*
