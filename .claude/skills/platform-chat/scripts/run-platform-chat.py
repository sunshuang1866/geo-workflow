...
assessments/{community}/{date}/responses.json after each question.
...
def run_question(script_path: Path, question_text: str, question_id: str,
                 timeout: int, env: dict, session: str = None) -> dict:
    ...
    try:
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=timeout * 2 + 90,  # login(~30s) + response(timeout) + follow-up(timeout) + buffer
            env=merged_env,
        )
...
def main():
    ...
    if args.questions:
        ...
        questions = [q for q in all_questions
                     if (q['question_id'], args.platform) not in done_set]
    ...
    if error_count > total * 0.5:
        print(f"\nWARNING: >50% questions failed ({error_count}/{total}). "
              "Check credentials or re-run with --questions for failed IDs.", file=sys.stderr)
