...
|------|----------|-------------|
| `questions.json` | `{community_dir}/` | Question set (generated by `/get-question`) |
...
5. Copy `{community_dir}/content-labels.json` to `{community_dir}/runs/{date}/content-labels.json`.
6. If `version_label` is not provided, derive it: count existing `runs/` subdirectories + 1, format as `V{n}`.
...
1. Update `run-meta.json` with completion data:
   ...
   ```
2. **Check for content-labels.json update actions**:
   ...
   c. If any exist, print a human action prompt:
      ```
      ⚠️  ACTION REQUIRED — content-labels.json 需要人工检查
...
packages/assessments/MindSpore/
  questions.json                <- question set (from /get-question, source of truth)
...
- All inputs (`questions.json`, `content-labels.json`, `.env`) must be pre-provisioned
